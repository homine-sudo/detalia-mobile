...
| `keiei.html` | **経営分析アプリ**(モバイル用・単一HTML・財務データはAES-256-GCM暗号化) | 編集可(下記§11) |
| `HANDOFF.md` | このファイル | — |
...
- [x] **事業カードらせんカルーセル＋ロゴ＋ことわざ全文**（2026-06-11 同日・ユーザー指示）→ ①事業カードに**各事業ロゴ**表示: レンタカー=`lime_logo_dark.png`(LIME) / ディテーリング=`lime_detailing_logo.png` / 電装=`navinchu_logo.png`(なびんちゅ・.clipで角丸) / 中古車=`ht_logo.png`(.circで円形)。ロゴ3点は Dropbox `システム一時保管` からASCII名でコピー・256px縮小済 ②**らせんカルーセル**: `.biz`(640px/perspective1600)内の `#helix` に4枚を rotateY 90°間隔+translateY段差(らせん)で配置、JS末尾のIIFEが自動回転(0.12deg/frame≒30秒/周)+ドラッグ回転(慣性付き・moved>10pxで誤クリック抑止)+hover一時停止。**≤860pxは縦積みフォールバック**(media queryで transform:none!important・JSはmq.matchesでスキップ) ③旧 .bcard マウスチルトIIFEは削除(helix と transform競合するため・復活させない) ④The Name に**ことわざ全文「瑠璃も玻璃も照らせば光る」**表示(ユーザー方針転換・「玻璃」「照らせ」=ハリテラス部分を .pw で強調・他は .pp 小さめ) ⑤連絡先の「（LIME DETAILING STUDIO）」削除 ⑥**トーン調整: ナルシスト感の排除**(ユーザー指示)=「妥協は許されない」「言い切れる」「決意を刻んだ」等の自賛調を事実ベース・謙抑に書き換え。**今後のコピーも自賛・断言調を避けること**
- [x] **全体磨き込み: 引き算の美学×IWC**（2026-06-11 同日・ユーザー「HP全体に引き算の美学・細部に神は宿る・IWCの世界観・美しく感動するHPに」）→ **削除**: マーキー帯(✕区切り)・ヒーローの量子方程式(.ket)・OBSERVEヒント(lighthint・CSS/JS/HTMLとも)・ブランドドット点滅・モバイルのHUD(display:none)。**新設**: `.railtrack`=IWC分目盛り風ディバイダー(1pxヘアライン2本+1pxティック14px間隔)を ヒーロー↓ と CTA↑ の2箇所。**静音化**: gridlines .5→.25 / grain .06→.04 / HUD opacity .72・薄ボーダー / helix-hint .6。**ヘアライン統一**: bcard/ctable/fbot の1.5px→1px・tag::before 2px→1px・prog 3px→2px。**余白**: section padding clamp(110px,14vw,200px)に拡大。設計原則=**アクセントはコバルト1色・罫線は1px・装飾は足すより引く**。会社概要の（）全廃も同方針(前項)。マーキー/ket/lighthint を復活させないこと
...
## 11. 経営分析アプリ `keiei.html`（2026-07-03 新規）

- **目的**: 社長がスマホで会社の財務全体像・ベンチマーク・シミュレーションを見られる経営分析アプリ。
- **データ源**: Drive `経営分析_引継ぎ_2026-05-21.md`（05-22更新反映）+ `財務分析レポート_A4_2026-05-22.pdf`。基準日 2026-05-22・第9期申告ベース。
- **🔐 社外秘対策**: 財務数値は本文に**平文で存在しない**。JSON全体を AES-256-GCM（PBKDF2-SHA256 310k iter）で暗号化した base64 ブロブとして埋め込み、閲覧時にパスフレーズでクライアント側復号。**パスフレーズはリポジトリに書かない**（社長へチャットで伝達済み。変更したい場合は `keiei_data.json` を再暗号化してブロブ差し替え）。
- **構成**: 5タブ = 概況（スコア83・KPI・リスク）/ 財務（PL・BS 4期・部門別・税）/ 比較（沖縄・全国・業界ベンチマーク+上級指標）/ 資産（車両77台・実質純資産ウォーターフォール・借入5本・売却実績・企業価値）/ 戦略（第10期予算シミュレーター・ディテーリング3層分析・技術者投資シミュレーター・アクションリスト）。
- **実装**: 外部依存ゼロの単一HTML。チャートは手書きSVG（dataviz手法準拠・ダーク面検証済みパレット gold `#AD8F3A` / blue `#3987e5` / aqua `#199e70` / violet `#9085e9` / red `#e66767`）。チェックリスト・「端末で記憶」は localStorage（キー `kh_key`, `kh_acts`）。
- **更新手順**: ①データ更新は JSON を編集 → node で PBKDF2+AES-GCM 暗号化 → `const BLOB="..."` を差し替え（前回スクリプト: セッションscratchpad `encrypt.js` 参照、形式 = base64(salt16|iv12|ct+tag16)）②UI はそのまま編集可 ③編集後は Playwright 等で復号→5タブ表示を確認。
- **第10期の月次実績が入ってきたら**: 予算シミュレーターの前提（実績反映版: 売上130M/純利益19.25M）と突き合わせる月次トラッキングを足すのが次の一手。
